...
```{r}
# Check for duplicate coordinates in the training data
sum(duplicated(train_data_sp@coords))

# Optionally, apply a stronger jitter if necessary to avoid overlapping points
train_data_sp@coords[, 1] <- jitter(train_data_sp@coords[, 1], factor = 0.001)
train_data_sp@coords[, 2] <- jitter(train_data_sp@coords[, 2], factor = 0.001)

```

```{r eval=FALSE}
# Now run the GWR prediction with jittered data
